...
class CourseDesignerInventoryPage extends StatefulWidget {
  ...
  @override
  State<CourseDesignerInventoryPage> createState() =>
      _CourseDesignerInventoryPageState();
}

class _CourseDesignerInventoryPageState
    extends State<CourseDesignerInventoryPage> {
      ...
      if (course == null || course.id == null) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
              content:
                  Text('No course selected. Please select a course first.')));
          setState(() {
  ...
  void _showManageTagsDialog(BuildContext context) {
    if (_courseId == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Course ID not available to manage tags.')));
      return;
    }
    showDialog(
      context: context,
  ...
  Widget build(BuildContext context) {
    if (_courseId == null &&
      ...
      return Scaffold(
        ...
        body: const Center(
            child: Text(
                'Course not available or error loading data. Please select a course and try again.')),
    ...
    return Scaffold(
      ...
      body: CustomUiConstants.framePage(
        // framePage provides SingleChildScrollView by default
        enableCreatorGuard: true,
        Column(
          // Removed explicit SingleChildScrollView
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Card(
              // Wrap Tags section in a Card
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  // Card takes minimum vertical space
                  children: [
                    Padding(
                      padding: const EdgeInsets.only(bottom: 8.0),
                      // Space between title and chips
                      child: Row(
                        children: [
                          Expanded(
                            child: Text('Tags',
                    ...
                    _isLoadingTags
                        ...
                        : (_tags.isEmpty)
                            ? const Text(
                                'No tags yet. Click the pencil to add some!')
                            : Wrap(
                                spacing: 8.0,
                                runSpacing: 8.0,
                                children: _tags.map((tag) {
                                  return Chip(
                                    ...
                                    backgroundColor: _parseColor(tag.color,
                                        defaultColor: Colors.blueGrey[100]!),
  ...
  void _addNewCategory() async {
    if (_courseId == null) {
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('Course not selected.')));
      return;
    }
    final name = _newCategoryNameController.text.trim();
    if (name.isNotEmpty) {
      try {
        await TeachableItemCategoryFunctions.addCategory(
            courseId: _courseId!, name: name);
  ...
  void _editCategoryName(
      BuildContext context, TeachableItemCategory category) async {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return ListTile(
          ...
          title: Row(
            // Option A for notes indicator
            // mainAxisSize: MainAxisSize.min, // Not needed if title takes full width
            children: [
              Expanded(
                // Use Expanded instead of Flexible for title text
                child: Column(
                  // Keep Column for name and tags
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Row(
                      // Row for item name and notes icon
                      children: [
                        ...
                        ...assignedTags.map((tag) {
                          return Chip(
                            ...
                            onDeleted: () async {
                              ...
                              try {
                                await TeachableItemFunctions
                                    .removeItemTagFromItem(
                                        itemId: item.id!,
                                        tagRef: FirebaseFirestore.instance.doc(
                                            'teachableItemTags/${tag.id}'));
                                if (mounted) {
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                        content: Text(
                                            'Tag "${tag.name}" removed from "${item.name}".')),
                                  );
                                }
                              } catch (e) {
                                if (mounted) {
                            ...
                            onSelected: (String selectedTagId) async {
                              ...
                              try {
                                await TeachableItemFunctions.assignTagToItem(
                                    itemId: item.id!,
                                    tagRef: FirebaseFirestore.instance.doc(
                                        'teachableItemTags/$selectedTagId'));
                                if (mounted) {
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                        content: Text(
                                            'Tag added to "${item.name}".')),
                                  );
                                }
                              } catch (e) {
                                if (mounted) {
  ...
  Widget _buildAddItemInput(String categoryId) {
    _newItemNameControllers.putIfAbsent(
  ...
  Future<void> _moveItemUp(
      TeachableItem item, List<TeachableItem> categoryItems) async {
    if (item.id == null) return;
  ...
  void _editItem(BuildContext context, TeachableItem item) {
    if (_courseId == null || item.id == null) {
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Item or Course ID not available.')));
      return;
    }
    showDialog(
      context: context,
  ...
  List<TeachableItemTag> _getTagsFromIds(
    ...
    return allTags.where((tag) {
      return tagIds.any(
          (tagIdRef) => tagIdRef is DocumentReference && tagIdRef.id == tag.id);
    }).toList();
  }
