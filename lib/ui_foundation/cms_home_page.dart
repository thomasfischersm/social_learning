...
class CmsHomePageState extends State<CmsHomePage> {
  ...
  /*void _testGoogleDocs() async {
    ...
    dprint('Start Google sign in');
    var account = await googleSignIn.signIn();
    dprint('finished Google sign in: ${account?.displayName}');

    if (account == null) {
      dprint('failed to login');
      return;
    }
    ...
    for (StructuralElement structuralElement in readDoc.body!.content!) {
      ...
      if (structuralElement.paragraph != null) {
        for (ParagraphElement paragraphElement
          ...
          dprint('-- ${paragraphElement.textRun!.content}');
        }
      }
    }
    ...
    dprint('create doc endIndex is $index');
    ...
    dprint('wrote timestamp to the Google doc');
...
void _testUpdateGoogleDoc(DocsApi docsApi) async {
  ...
  if (updateRequests.isNotEmpty) {
    await docsApi.documents.batchUpdate(
        ...
        magicDoc.documentId!);
    dprint('wrote sums to the Google magicDoc');
  } else {
    dprint('no updates to make!');
  }
}
